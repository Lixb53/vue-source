...
export function initExtend (Vue: GlobalAPI) {
   ...
   * 扩展石可以传递一些默认配置, 就像 Vue 也会有一些默认配置
  ...
  Vue.extend = function (extendOptions: Object): Function {
    ...
    // 什么情况下会用到缓存?
    //  如果在多次使用 Vue.extend 时使用了同一个配置项, 这是就会启用该缓存
    ...
    // 在组件内可以通过 this.computedKey 访问
    if (Sub.options.computed) {
      initComputed(Sub)
    }

    // allow further extension/mixin/plugin usage
    // 定义 extend, mixin, use 这三个静态方法, 允许在 Sub 基础上再进一步进行扩展
